...
static int xhci_stream_context_array_show(struct seq_file *s, void *unused)
{
	...
	for (id = 0; id < epriv->stream_info->num_stream_ctxs; id++) {
		...
		ctx_dma = epriv->stream_info->ctx_array_dma + id * 16;

		if (id < epriv->stream_info->num_streams) {
			ctx = le64_to_cpu(stream_ctx->stream_ring);
			deq_dma = ctx & TR_DEQ_PTR_MASK;
			seq_printf(s, "@%pad stream %d: deq @%pad SCT %llu cycle %llu\n",
				   &ctx_dma, id, &deq_dma, CTX_TO_SCT(ctx),
