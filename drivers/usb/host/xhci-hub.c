		...
		u32 *status, u32 status_reg)
{
	...
	if (status_reg & PORT_CAS) {
		 ...
		 */
		if ((xhci->quirks & XHCI_COMP_MODE_QUIRK) && (pls == PLS_COMP_MODE))
			pls |= USB_PORT_STAT_CONNECTION;
	}
				    ...
				    u16 wIndex)
{
	...
	bool port_in_u0 = ((status & PORT_PLS_MASK) == PLS_U0);
	...
	__acquires(&xhci->lock)
{
	...
	if ((raw_port_status & PORT_PRC))
		status |= USB_PORT_STAT_C_RESET << 16;

	/* common wPortStatus bits */
	if (raw_port_status & PORT_CCS) {
		status |= USB_PORT_STAT_CONNECTION;
		status |= xhci_port_speed(raw_port_status);
	}
	if (raw_port_status & PORT_PED)
		status |= USB_PORT_STAT_ENABLE;
	if (raw_port_status & PORT_OCA)
		status |= USB_PORT_STAT_OVERCURRENT;
	if (raw_port_status & PORT_PR)
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_SUSPEND:
			portsc = xhci_portsc_readl(port);
			if ((portsc & PORT_PLS_MASK) != PLS_U0) {
	...
	case ClearPortFeature:
		...
		case USB_PORT_FEAT_C_PORT_LINK_STATE:
		case USB_PORT_FEAT_C_PORT_CONFIG_ERROR:
			xhci_clear_port_change_bit(xhci, wValue, wIndex, port, portsc);
			break;
...
int xhci_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	/* All RW1CS bits except for PORT_PED and PORT_PRC */
...
retry:
		...
		if ((t1 & PORT_PED) && (t1 & PORT_PLS_MASK) == PLS_U0) {
...
static bool xhci_port_missing_cas_quirk(struct xhci_port *port)
{
	...
	if (((portsc & PORT_PLS_MASK) != PLS_POLLING) &&
	    ((portsc & PORT_PLS_MASK) != PLS_COMP_MODE))
		return false;

	/* clear wakeup/change bits, and do a warm port reset */
	portsc &= ~(PORTSC_RW1CS | PORT_WAKE_BITS);
	portsc |= PORT_WPR;
	xhci_portsc_writel(port, portsc);
	/* flush write */
	xhci_portsc_readl(port);
