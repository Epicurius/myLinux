...
static void xhci_disable_port(struct xhci_hcd *xhci, struct xhci_port *port)
{
	...
	writel(portsc | PORT_PE, &port->addr->portsc);
...
static void xhci_clear_port_change_bit(struct xhci_hcd *xhci, u16 wValue, u16 wIndex,
				       struct xhci_port_regs __iomem *port_regs, u32 port_status)
{
	...
	writel(port_status | status, &port_regs->portsc);
	port_status = readl(&port_regs->portsc);
	...
	__must_hold(&xhci->lock)
{
	...
	if (on) {
		/* Power on */
		writel(portsc | PORT_POWER, &port->addr->portsc);
	...
	spin_unlock_irqrestore(&xhci->lock, *flags);
	portsc = usb_acpi_power_manageable(hcd->self.root_hub,
					 port->hcd_portnum);
	if (portsc)
				      ...
				      struct xhci_port *port, u16 wake_mask)
{
	...
	if (wake_mask & USB_PORT_FEAT_REMOTE_WAKE_CONNECT)
		portsc |= PORT_WKCONN_E;
	else
		portsc &= ~PORT_WKCONN_E;

	if (wake_mask & USB_PORT_FEAT_REMOTE_WAKE_DISCONNECT)
		portsc |= PORT_WKDISC_E;
	else
		portsc &= ~PORT_WKDISC_E;
				    ...
				    u16 wIndex)
{
	...
	bool port_in_u0 = ((status & PORT_PLS_MASK) == USB_SS_PORT_LS_U0);
				      ...
				      u32 portsc)
{
	...
	if (portsc & PORT_PLC && (link_state != XHCI_PLS_RESUME))
	...
	if (link_state != USB_SS_PORT_LS_U3 &&
	    link_state != XHCI_PLS_RESUME &&
	    link_state != USB_SS_PORT_LS_RECOVERY) {
				      ...
				      u32 portsc, unsigned long *flags)
{
	...
	if (portsc & PORT_POWER) {
		*status |= USB_PORT_STAT_POWER;
		...
		if (link_state == USB_SS_PORT_LS_U2)
	 ...
	 */
	if (link_state != USB_SS_PORT_LS_U3 && link_state != XHCI_PLS_RESUME) {
		if (port->resume_timestamp ||
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case GetPortStatus:
		...
		status = xhci_get_port_status(hcd, bus_state, wIndex, portsc,
					      &flags);
		if (status == 0xffffffff)
			goto error;

		xhci_dbg(xhci, "Get port status %d-%d read: 0x%x, return 0x%x",
			 hcd->self.busnum, portnum1, portsc, status);
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_SUSPEND:
			portsc = readl(&port->addr->portsc);
			if ((portsc & PORT_PLS_MASK) != USB_SS_PORT_LS_U0) {
			...
			if ((portsc & PORT_PE) == 0 || (portsc & PORT_RESET)
				|| (portsc & PORT_PLS_MASK) >= USB_SS_PORT_LS_U3) {
		...
		case USB_PORT_FEAT_LINK_STATE:
			...
			if (link_state == USB_SS_PORT_LS_U0) {
				...
				/* already in U0 */
				if (pls == USB_SS_PORT_LS_U0)
					break;
				if (pls == USB_SS_PORT_LS_U3 ||
				    pls == XHCI_PLS_RESUME ||
				    pls == USB_SS_PORT_LS_RECOVERY) {
					wait_u0 = true;
					reinit_completion(&port->u3exit_done);
				}
				if (pls <= USB_SS_PORT_LS_U3) /* U1, U2, U3 */
					xhci_set_link_state(xhci, port, USB_SS_PORT_LS_U0);
				if (!wait_u0) {
					if (pls > USB_SS_PORT_LS_U3)
						goto error;
		...
		case USB_PORT_FEAT_RESET:
			...
			xhci_dbg(xhci, "set port reset, actual port %d-%d status  = 0x%x\n",
				 hcd->self.busnum, portnum1, portsc);
			break;
	...
	case ClearPortFeature:
		...
		case USB_PORT_FEAT_SUSPEND:
			...
			if ((portsc & PORT_PLS_MASK) == USB_SS_PORT_LS_U3) {
				...
				set_bit(wIndex, &bus_state->resuming_ports);
				usb_hcd_start_port_resume(&hcd->self, wIndex);
				xhci_set_link_state(xhci, port, XHCI_PLS_RESUME);
				spin_unlock_irqrestore(&xhci->lock, flags);
...
retry:
		...
		if ((hcd->speed >= HCD_USB3) && retries-- &&
		    (t1 & PORT_PLS_MASK) == USB_SS_PORT_LS_POLLING) {
			spin_unlock_irqrestore(&xhci->lock, flags);
		...
		if ((t1 & PORT_PE) && (t1 & PORT_PLS_MASK) == USB_SS_PORT_LS_U0) {
			...
			xhci_dbg(xhci, "port %d-%d not suspended\n",
				 hcd->self.busnum, port_index + 1);
			t2 &= ~PORT_PLS_MASK;
			t2 |= PORT_LINK_STROBE | USB_SS_PORT_LS_U3;
	...
	while (port_index--) {
		...
		writel(portsc_buf[port_index], &ports[port_index]->addr->portsc);
	}
...
static bool xhci_port_missing_cas_quirk(struct xhci_port *port)
{
	...
	if (((portsc & PORT_PLS_MASK) != USB_SS_PORT_LS_POLLING) &&
	    ((portsc & PORT_PLS_MASK) != USB_SS_PORT_LS_COMP_MOD))
		return false;
...
int xhci_bus_resume(struct usb_hcd *hcd)
{
	...
	if (hcd->speed >= HCD_USB3) {
		next_state = USB_SS_PORT_LS_U0;
	} else {
		next_state = XHCI_PLS_RESUME;
		if (bus_state->bus_suspended) {
			/*
	...
	while (port_index--) {
		...
		portsc &= ~(PORTSC_RW1CS_BITS | PORT_WAKE_BITS);
		writel(portsc, &ports[port_index]->addr->portsc);
	}
	...
	if (hcd->speed < HCD_USB3) {
		...
		for_each_set_bit(port_index, &bus_state->bus_suspended,
			...
			xhci_test_and_clear_bit(xhci, ports[port_index],
						PORT_PLC);
			xhci_set_link_state(xhci, ports[port_index], USB_SS_PORT_LS_U0);
		}
	}
