...
static void xhci_disable_port(struct xhci_hcd *xhci, struct xhci_port *port)
{
	...
	portsc = readl(&port->port_regs->portsc);
	portsc = xhci_port_state_to_neutral(portsc);

	/* Write 1 to disable the port */
	writel(portsc | PORT_PE, &port->port_regs->portsc);
	...
	__must_hold(&xhci->lock)
{
	...
	hcd = port->rhub->hcd;
	temp = readl(&port->port_regs->portsc);
	...
	if (on) {
		/* Power on */
		writel(temp | PORT_POWER, &port->port_regs->portsc);
		readl(&port->port_regs->portsc);
	} else {
		/* Power off */
		writel(temp & ~PORT_POWER, &port->port_regs->portsc);
	}
	...
	u16 test_mode, u16 wIndex)
{
	...
	temp = readl(&port->port_regs->portpmsc);
	temp |= test_mode << PORT_TEST_MODE_SHIFT;
	writel(temp, &port->port_regs->portpmsc);
			 ...
			 u32 link_state)
{
	...
	writel(temp, &port->port_regs->portsc);
					     ...
					     unsigned long *flags)
{
	...
	/* Has resume been signalled for USB_RESUME_TIME yet? */
		...
		if (time_left) {
			...
			int port_status = readl(&port->port_regs->portsc);
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case GetPortStatus:
		...
		if (wValue == 0x02) {
			...
			port_li = readl(&port->port_regs->portli);
		...
		case USB_PORT_FEAT_LINK_STATE:
			...
			if (link_state == USB_SS_PORT_LS_SS_DISABLED) {
				...
				temp |= PORT_CSC | PORT_PEC | PORT_WRC |
					...
					PORT_CEC;
				writel(temp | PORT_PE, &port->port_regs->portsc);
				temp = readl(&port->port_regs->portsc);
				break;
			}
...
int xhci_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	for (i = 0; i < max_ports; i++) {
		temp = readl(&ports[i]->port_regs->portsc);
		if (temp == ~(u32)0) {
			xhci_hc_died(xhci);
			retval = -ENODEV;
			break;
...
int xhci_bus_suspend(struct usb_hcd *hcd)
{
	...
	while (port_index--) {
		...
		int retries = 10;
retry:
		t1 = readl(&ports[port_index]->port_regs->portsc);
	...
	while (port_index--) {
		...
		writel(portsc_buf[port_index], &ports[port_index]->port_regs->portsc);
	}
...
static bool xhci_port_missing_cas_quirk(struct xhci_port *port)
{
	...
	portsc |= PORT_WR;
	writel(portsc, &port->port_regs->portsc);
	/* flush write */
	readl(&port->port_regs->portsc);
	return true;
}
...
int xhci_bus_resume(struct usb_hcd *hcd)
{
	...
	while (port_index--) {
		...
		portsc &= ~(PORT_RWC_BITS | PORT_CEC | PORT_WAKE_BITS);
		writel(portsc, &ports[port_index]->port_regs->portsc);
	}
	...
	for_each_set_bit(port_index, &bus_state->bus_suspended, BITS_PER_LONG) {
		sret = xhci_handshake(&ports[port_index]->port_regs->portsc, PORT_PLC,
				      PORT_PLC, 10 * 1000);
		if (sret) {
