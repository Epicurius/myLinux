...
static void xhci_free_segments_for_ring(struct xhci_hcd *xhci, struct list_head *seg_list)
{
	...
	list_for_each_entry_safe(seg, _seg, seg_list, list) {
		list_del(&seg->list);
			    ...
			    struct list_head *new_list, unsigned int num_segs)
{
	...
	if (list_is_last(&new_list_last->list, &ring->seg_list)) {
		xhci_set_link_trb(new_list_last, _get_first_seg(ring), chain_links);
		...
		enum xhci_ring_type type, unsigned int max_packet, gfp_t flags)
{
	...
	if (type != TYPE_EVENT) {
		...
		last_seg = _get_last_seg(ring);
		last_seg->trbs[TRBS_PER_SEGMENT - 1].link.control |= cpu_to_le32(LINK_TOGGLE);
	}
...
{
	/*
	 * 'new_list' head can be stack-allocated as it's discarded upon
	 * integration into the ring.
	 */
	LIST_HEAD(new_list);
	int ret;
