...
static bool xhci_hub_ports_suspended(struct xhci_hub *hub)
{
	...
	for (i = 0; i < hub->num_ports; i++) {
		value = readl(hub->ports[i]->port_regs->portsc);
		if ((value & PORT_PE) == 0)
			continue;
...
static void tegra_xhci_enable_phy_sleepwalk_wake(struct tegra_xusb *tegra)
{
	...
	for (i = 0, k = 0; i < tegra->soc->num_types; i++) {
		...
		for (j = 0; j < tegra->soc->phy_types[i].num; j++) {
			...
			portsc = readl(rhub->ports[index]->port_regs->portsc);
...
static int tegra_xusb_enter_elpg(struct tegra_xusb *tegra, bool is_auto_resume)
{
	...
	for (i = 0; i < xhci->usb2_rhub.num_ports; i++) {
		if (!xhci->usb2_rhub.ports[i])
			continue;
		portsc = readl(xhci->usb2_rhub.ports[i]->port_regs->portsc);
	...
	if (bus_state->resuming_ports && hcd->speed == HCD_USB2) {
		...
		while (i--) {
			if (!test_bit(i, &bus_state->resuming_ports))
				continue;
			portsc = readl(&ports[i]->port_regs->portsc);
	...
	if (hcd->speed == HCD_USB2) {
		...
		if ((type_req == SetPortFeature) && (value == USB_PORT_FEAT_RESET)) {
			if (!index || index > rhub->num_ports)
				return -EPIPE;
			ports = rhub->ports;
			portsc = readl(&ports[port]->port_regs->portsc);
