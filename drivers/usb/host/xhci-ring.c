...
/*
 * If the suspect DMA address is a TRB is within range of 'start_trb' in 'start_seg' to
 * 'end_trb' in 'end_seg', this function returns that TRB's segment. Otherwise it returns 0.
 */
static struct xhci_segment *dma_in_range(struct xhci_ring *ring, dma_addr_t dma,
...
{
	return dma_in_range(ring, dma, td->start_seg, td->start_trb, td->end_seg, td->end_trb);
}

static struct xhci_segment *trb_in_queue(struct xhci_ring *ring, dma_addr_t dma)
{
	return dma_in_range(ring, dma, ring->deq_seg, ring->dequeue, ring->enq_seg, ring->enqueue);
}
				  ...
				  unsigned int ep_index, unsigned int slot_id)
{
	...
	case COMP_RING_OVERRUN:
		...
		break;
	/* Completion codes indicating some isoc TDs were missed */
 ...
 */
static unsigned int trb_idx_unwrap(struct xhci_segment *seg, union xhci_trb *trb,
					  ...
					  union xhci_trb *trb, int status)
{
	...
	min_idx = trb_idx_unwrap(ring->deq_seg, ring->dequeue, trbs, 0);
	trb_idx = trb_idx_unwrap(seg, trb, trbs, min_idx);

	list_for_each_entry_safe(td, _td, &ring->td_list, td_list) {
		/* If the TD is before the target TRB, skip it. */
		if (trb_idx_unwrap(td->end_seg, td->end_trb, trbs, min_idx) < trb_idx) {
		...
		if (trb_idx < trb_idx_unwrap(td->start_seg, td->start_trb, trbs, min_idx))
			return NULL;
